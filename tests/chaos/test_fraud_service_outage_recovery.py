...
async def test_fraud_service_outage_recovers_within_nfr11_budget(merchant_token):
    ...
    assert result.recovered, f"scenario did not resolve within {RESOLUTION_TIMEOUT_SECONDS}s: {result.detail}"
    assert result.resolve_seconds is not None and result.resolve_seconds < 120, (
        f"NFR11 requires MTTR < 120s in the local/demo environment; got {result.resolve_seconds:.1f}s ({result.detail})"
    )
