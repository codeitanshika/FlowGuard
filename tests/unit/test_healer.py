...
@pytest.mark.parametrize(
    "a",
    [
        anomaly(service="gateway"),
        anomaly(service="notification"),
        anomaly(metric="p95_latency", value=2000),
        anomaly(metric="throughput", value=0),
    ],
)
...
def test_plan_executes_valid_decision():
    ...
    assert (p.kind, p.action, p.service, p.dependency, p.validated) == (
        "execute",
        "open-circuit",
        "payment",
        "provider",
...
GOOD = json.dumps(
    {
        "root_cause": "provider is failing",
        "confidence": "high",
        "action": "open-circuit",
        "service": "payment",
        "dependency": "provider",
        "reasoning": "trace names provider",
    }
)
